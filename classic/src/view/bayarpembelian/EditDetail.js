Ext.define('Admin.view.bayarpembelian.EditDetail', {
    extend: 'Ext.grid.Panel',
    xtype: 'bayarpembelian-edit-detail',

    cls: 'bayarpembelian-grid',
    constructor: function(c) {
        ...
        var invoiceSelect = function(f, record) {
            ...
            store.getAt(index).set('id_pembelian', record.data['id']); //id_pembelian
            store.getAt(index).set('detail_akun', record.data['detail_akun']); //detail_akun

            me.columns[1].getEditor(rec).setValue(record.data['notrx']); //notrx_pembelian
        ...
        me.rowEditor = Ext.create('Ext.grid.plugin.RowEditing', {
            ...
            listeners: {
                ...
                beforeedit: function(f, e) {
                    ...
                    invoice.load({
                        params: {
                            id_vendor: me.up('bayarpembelian-edit').down('#subyek').getSubmitValue()
                        }
                    });
        ...
        Ext.apply(c, {
            ...
            columns: [
                ...
                //--- [1]
                {xtype: 'gridcolumn', cls: 'content-column', text: 'No. Invoice', flex: 0.7, menuDisabled: true, sortable: false, align: 'left', dataIndex: 'notrx_pembelian', 
                    editor: {
                        xtype: 'combobox',
                        store: Ext.create('Ext.data.Store', {
                            ...
                            proxy: {
                                type: 'ajax',
                                url: './server/public/pembelian/all',
                                reader: {
                                    type: 'json',
                                    rootProperty: 'data'
                                }
    ...
    getTotal: function() {
        ...
        me.up('bayarpembelian-edit').down('#total').setValue(total);
    },
    ...
    getDetail: function() {
        ...
        for(var row =0;row<me.getStore().getCount();row++) {
            ...
            str += (str!=''?';':'') +
                rec.data['id_pembelian'] + ',' +
                rec.data['notrx_pembelian'] + ',' +                     
                rec.data['kode_akun'] + ',' +
