Ext.define('Admin.view.bayarpenjualan.BayarPenjualanController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.bayarpenjualan',

    init: function() {
        var me = this;
        var viewType = me.getView().viewType;
        if(viewType && viewType!='webdesktop') {
            console.log('bayarpenjualan-init');
    ...
    onAddButtonClick: function(btn) {
        ...
        if(viewType=='webdesktop') {   
            var params = {
                ...
                title: 'Tambah Kas & Bank Masuk',
                modulId: 'bayarpenjualan-add',
                modal: true,
                constrain: true,
                grid: gridpanel
            };
            ...
            this.setCurrentView('bayarpenjualan-edit', {
    ...
    onEditButtonClick: function(btn) {
        ...
        if(viewType=='webdesktop') {   
            var params = {
                xtype: 'webdesktop-bayarpenjualan-edit',
                title: 'Edit Kas & Bank Masuk',
                modulId: 'bayarpenjualan-edit',
                modal: true,
                constrain: true,
                grid: gridpanel,
    ...
    onLoadData: function(form) {
        var win = this.getView();
        var viewType = win.viewType;
        var form = win.down('bayarpenjualan-edit');
        ...
        if(idEdit) {
            form.load({
                //waitMsg: 'Loading...',
                method: 'GET',
                url: './server/public/bayarpenjualan/'+idEdit+'/load',
                success: function (frm, action) {
                    var json = Ext.JSON.decode(action.response.responseText);
                    var grid = form.down('bayarpenjualan-edit-detail');
    ...
    onDeleteButtonClick: function(btn) {
        ...
        Ext.MessageBox.confirm('Konfirmasi', 'Yakin untuk hapus data yang dipilih?', function(btn,text) {
            if(btn=='yes') {
                ...
                Ext.Ajax.request({
                    method:'GET',
                    url: './server/public/bayarpenjualan/'+gridpanel.idSelect+'/delete',
    ...
    onSaveButtonClick: function(btn) {
        ...
        var editDetail = form.down('bayarpenjualan-edit-detail');
        ...
        if(form.getForm().isValid()) {
            Ext.MessageBox.confirm('Konfirmasi', 'Yakin untuk proses simpan data?', function(btn, text) {
                if(btn=='yes') {
                    ...
                    form.getForm().submit({
                        method:'POST',
                        url: './server/public/bayarpenjualan/'+saveUrl,
                        params: {
