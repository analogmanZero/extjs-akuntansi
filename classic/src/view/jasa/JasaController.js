Ext.define('Admin.view.jasa.JasaController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.jasa',

    init: function() {
        var me = this;
        var viewType = me.getView().viewType;
        if(viewType && viewType!='webdesktop') {
            console.log('jasa-init');
    ...
    onLoadData: function(form) {
        ...
        if(idEdit) {
            form.load({
                //waitMsg: 'Loading...',
                method: 'GET',
                url: './server/public/jasa/'+idEdit+'/load',
    ...
    onDeleteButtonClick: function(btn) {
        ...
        Ext.MessageBox.confirm('Konfirmasi', 'Yakin untuk hapus data yang dipilih?', function(btn,text) {
            if(btn=='yes') {
                ...
                Ext.Ajax.request({
                    method:'GET',
                    url: './server/public/jasa/'+gridpanel.idSelect+'/delete',
        ...
        if(form.getForm().isValid()) {
            Ext.MessageBox.confirm('Konfirmasi', 'Yakin untuk proses simpan data?', function(btn, text) {
                if(btn=='yes') {
                    ...
                    form.getForm().submit({
                        method:'POST',
                        url: './server/public/jasa/'+saveUrl,
                        waitMsg: 'Simpan...',
                        success:function(frm, action) {
                            var json = Ext.JSON.decode(action.response.responseText);
                            Ext.Msg.alert('Sukses', json['message'], function(btn, text) {
    ...
    onPajakJualCheckChange: function(column, rowIndex, checked, eOpts) {
        ...
        Ext.Ajax.request({
            method:'POST',
            url: './server/public/jasa/'+idEdit+'/pajakjual',
            params: {
                status: checked?1:0
            },
            success: function(response) {
    ...
    onPajakBeliCheckChange: function(column, rowIndex, checked, eOpts) {
        ...
        Ext.Ajax.request({
            method:'POST',
            url: './server/public/jasa/'+idEdit+'/pajakbeli',
            params: {
                status: checked?1:0
            },
            success: function(response) {
