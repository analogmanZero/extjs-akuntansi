Ext.define('Admin.view.kasbankmasuk.KasBankMasukController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.kasbankmasuk',

    init: function() {
        var me = this;
        var viewType = me.getView().viewType;
        if(viewType && viewType!='webdesktop') {
            console.log('kasbankmasuk-init');
    ...
    onAddButtonClick: function(btn) {
        ...
        if(viewType=='webdesktop') {   
            ...
            this.setCurrentView('kasbankmasuk-edit', {
    ...
    onLoadData: function(form) {
        var win = this.getView();
        var viewType = win.viewType;
        var form = win.down('kasbankmasuk-edit');
        ...
        if(idEdit) {
            form.load({
                //waitMsg: 'Loading...',
                method: 'GET',
                url: './server/public/kasbankmasuk/'+idEdit+'/load',
                success: function (frm, action) {
                    var json = Ext.JSON.decode(action.response.responseText);
                    var grid = form.down('kasbankmasuk-edit-detail');
    ...
    onDeleteButtonClick: function(btn) {
        ...
        Ext.MessageBox.confirm('Konfirmasi', 'Yakin untuk hapus data yang dipilih?', function(btn,text) {
            if(btn=='yes') {
                ...
                Ext.Ajax.request({
                    method:'GET',
                    url: './server/public/kasbankmasuk/'+gridpanel.idSelect+'/delete',
    ...
    onSaveButtonClick: function(btn) {
        ...
        var editDetail = form.down('kasbankmasuk-edit-detail');
        ...
        if(form.getForm().isValid()) {
            Ext.MessageBox.confirm('Konfirmasi', 'Yakin untuk proses simpan data?', function(btn, text) {
                if(btn=='yes') {
                    ...
                    form.getForm().submit({
                        method:'POST',
                        url: './server/public/kasbankmasuk/'+saveUrl,
                        params: {
