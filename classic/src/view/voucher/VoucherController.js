Ext.define('Admin.view.voucher.VoucherController', {
    ...
    onAddButtonClick: function(btn) {
        ...
        if(viewType=='webdesktop') {   
            ...
            this.setCurrentView('voucher-edit', {
    ...
    onLoadData: function(form) {
        var win = this.getView();
        var viewType = win.viewType;
        var form = win.down('voucher-edit');
        ...
        if(idEdit) {
            form.load({
                //waitMsg: 'Loading...',
                method: 'GET',
                url: './server/public/voucher/'+idEdit+'/load',
    ...
    onDeleteButtonClick: function(btn) {
        ...
        Ext.MessageBox.confirm('Konfirmasi', 'Yakin untuk hapus data yang dipilih?', function(btn,text) {
            if(btn=='yes') {
                ...
                Ext.Ajax.request({
                    method:'GET',
                    url: './server/public/voucher/'+gridpanel.idSelect+'/delete',
        ...
        if(form.getForm().isValid()) {
            Ext.MessageBox.confirm('Konfirmasi', 'Yakin untuk proses simpan data?', function(btn, text) {
                if(btn=='yes') {
                    ...
                    form.getForm().submit({
                        method:'POST',
                        url: './server/public/voucher/'+saveUrl,
                        waitMsg: 'Simpan...',
                        success:function(frm, action) {
                            var json = Ext.JSON.decode(action.response.responseText);
                            Ext.Msg.alert('Sukses', json['message'], function(btn, text) {
    ...
    onPajakJualCheckChange: function(column, rowIndex, checked, eOpts) {
        ...
        Ext.Ajax.request({
            method:'POST',
            url: './server/public/voucher/'+idEdit+'/pajakjual',
            params: {
                status: checked?1:0
            },
            success: function(response) {
