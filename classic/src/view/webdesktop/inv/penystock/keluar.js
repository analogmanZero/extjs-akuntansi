Ext.define('Admin.view.webdesktop.inv.penystock.keluar', {

    extend: 'Ext.grid.Panel',
    alias : 'widget.invpenystockkeluar',
    ...
    constructor: function(c) {
        ...
        Ext.apply(c, {
            ...
            store: Ext.create('Admin.store.stores', {
                fields: ['id', 'kode_barang', 'nama_barang', 'bentuk', 'noroll', 'substance', 'size', 'ukuran_potong', 'qty', 'satuan', 'harga', 'id_stok_masuk', {name: 'jumlah', type: 'int'}],
                proxy: {
                    type: 'memory',
                    reader: {
                        type: 'json',
                        rootProperty: 'keluars'
                    }
                }
            }),
                ...
                {sortable: false, text: 'No. Roll', dataIndex: 'noroll', align: 'left', flex: 0.8, menuDisabled: true,
                    editor: {
                        ...
                        store: Ext.create('Admin.store.stores', {
                            ...
                            autoLoad: true,
                            params: {notrx: c.isEdit, sumber: 'PY'}
                        }),
                        valueField: 'noroll',
                        displayField: 'noroll',
                        queryMode: 'local',
                        ...
                        listeners: {
                            select: function(c, records) {
                                var substance = me.columns[4].getEditor(me.recordSelected);
                                var size = me.columns[5].getEditor(me.recordSelected);
    ...
    penjelasanChanges: function(f) {
        ...
        var noroll = grid.columns[3].getEditor(record);
    ...
    getDetail: function() {
        ...
        for(var row =0;row<me.getStore().getCount();row++) {
            ...
            str += (str!=''?'\n':'') +                                //  id keterangan
                ...
                rec.data['id_stok_masuk'];                            //  12 id_stok_masuk
        }
