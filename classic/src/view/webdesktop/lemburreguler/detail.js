Ext.define('Admin.view.webdesktop.lemburreguler.detail', {

    extend: 'Ext.grid.Panel',
    alias : 'widget.lemburregulerdetail',
    ...
    constructor: function(c) {
        ...
        var storeKaryawan = Ext.create('Admin.store.stores', {
            ...
            url: 'api/store/lemburreguler/karyawanStore.php',
            pageSize: 1000000
        });
        ...
        me.store = Ext.create('Admin.store.stores', {
            fields: ['id', 'nik', 'nama', 'area', 'tanggal', {name: 'nilai', type: 'float'}, 'jenis', {name: 'qty', type: 'float'}, {name: 'total', type: 'float'}, {name: 'jumlah', type: 'float'}], //{name: 'satu', type: 'float'}, {name: 'dua', type: 'float'}, {name: 'tiga', type: 'float'}, 
            url: 'api/store/lemburreguler/dataStore.php',
            proxy: {
                type: 'memory',
                reader: {
                    type: 'json',
        ...
        me.rowEditor = Ext.create('Ext.grid.plugin.RowEditing', {
            ...
            listeners: {
                ...
                afteredit: function(f, e) {
                    var temp = me.columns[5].getEditor(e.record).getSubmitValue().split('-');
        ...
        Ext.apply(c, {
            ...
            columns: [
                ...
                //--- [1]
                {sortable: false, text: 'NIK', dataIndex: 'nik', width: 70, menuDisabled: true, align: 'center',
                    editor: {
                        xtype: 'combobox',
                        store: storeKaryawan,
                        valueField: 'nik',
                        ...
                        listeners: {
                            ...
                            select: function(combo, record) {
                                ...
                                var nilai = me.columns[4].getEditor(me.recordSelected);
                ...
                //--- [6]
                {sortable: false, text: 'Jenis', dataIndex: 'jenis', align: 'center', width: 120, menuDisabled: true,
                    editor: {
                        xtype: 'combobox',
                        store: Ext.create('Ext.data.Store', {
                            fields: ['nama'],
                            data:[{nama: 'REGULER'}, {nama: 'BACKUP'}, {nama: 'PH'}]
                        }),
                        valueField: 'nama',
                        displayField: 'nama',
                        listeners: {
                        ...
                        listeners: {
                            specialkey: function(field, e){
                                ...
                                if (e.getKey() == e.ENTER) {
                                    ...
                                    var jenis = me.columns[6].getEditor(me.recordSelected).getSubmitValue();
                                    var jam   = me.columns[7].getEditor(me.recordSelected).getSubmitValue();
                                    var pecah = [0, 0, 0];
                                    var pengurang = [jenis=='REGULER'?1:0, 7];
                                    var pengali = [1.5, 2, 3];
                                    ...
                                    Ext.Ajax.request({
                                        url: 'api/store/lemburreguler/save.php',
                                        method: 'POST',
                                        params:{
                                            ...
                                            tanggal: me.columns[5].getEditor(me.recordSelected).getSubmitValue(),
                                            jenis: jenis,
    ...
    hapusRecord: function() {
        ...
        if (selection) {
            Ext.MessageBox.confirm('Konfirmasi', 'Yakin untuk hapus data yang dipilih?', function(btn,text) {
                if(btn=='yes') {
                    Ext.Ajax.request({
                        url: 'api/store/lemburreguler/delete.php',
                        method: 'POST',
                        params:{
                            id: selection.get('id')                    
                        },
    ...
    getJumlah: function() {
        ...
        setTimeout(function() {
            ...
            me.columns[9].getEditor(me.recordSelected).setValue((eval(nilai)*eval(total)));
