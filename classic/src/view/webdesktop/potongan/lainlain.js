Ext.define('Admin.view.webdesktop.potongan.lainlain', {

    extend: 'Ext.grid.Panel',
    alias : 'widget.potonganlainlain',
    ...
    constructor: function(c) {
        ...
        me.store = Ext.create('Admin.store.stores', {
            fields: ['id', 'nik', 'nama', 'status_pegawai', 'area', 'bulan', 'keterangan', {name: 'nilai', type: 'float'}], //{name: 'satu', type: 'float'}, {name: 'dua', type: 'float'}, {name: 'tiga', type: 'float'}, 
            url: 'api/store/potongan/lainlainStore.php',
            proxy: {
                type: 'memory',
                reader: {
                    type: 'json',
        ...
        Ext.apply(c, {
            ...
            columns: [
                ...
                //--- [5]
                {sortable: false, text: 'Periode', dataIndex: 'bulan', align: 'center', width: 120, menuDisabled: true,
                    editor: {
                        xtype: 'monthfield',
                        fieldStyle: 'text-align: center;',
                        format: 'm-Y',
                ...
                //--- [6]
                {sortable: false, text: 'Keterangan', dataIndex: 'keterangan', flex: 1.5, menuDisabled: true, align: 'left',
                    editor: {
                        xtype: 'textfield',
                        listeners: {
                            specialkey: function(field, e){
                                ...
                                if (e.getKey() == e.ENTER) {
                                    ...
                                    Ext.Ajax.request({
                                        url: 'api/store/potongan/lainlainSave.php',
                                        method: 'POST',
                                        params:{
                                            ...
                                            bulan: me.columns[5].getEditor(me.recordSelected).getSubmitValue()+'-01',
                                            keterangan: me.columns[6].getEditor(me.recordSelected).getSubmitValue()
                                        },
    ...
    hapusRecord: function() {
        ...
        if (selection) {           
            Ext.MessageBox.confirm('Konfirmasi', 'Yakin untuk hapus data yang dipilih?', function(btn,text) {
                if(btn=='yes') {
                    Ext.Ajax.request({
                        url: 'api/store/potongan/lainlainDelete.php',
                        method: 'POST',
                        params:{
                            id: selection.get('id')                    
                        },
