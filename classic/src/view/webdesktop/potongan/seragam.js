Ext.define('Admin.view.webdesktop.potongan.seragam', {

    extend: 'Ext.grid.Panel',
    alias : 'widget.potonganseragam',
    ...
    constructor: function(c) {
        ...
        me.store = Ext.create('Admin.store.stores', {
            fields: ['id', 'nik', 'nama', 'status_pegawai', 'area', {name: 'nilai', type: 'float'}, 'bulan', {name: 'tempo', type: 'float'} ], //{name: 'satu', type: 'float'}, {name: 'dua', type: 'float'}, {name: 'tiga', type: 'float'}, 
            url: 'api/store/potongan/seragamStore.php',
            proxy: {
                type: 'memory',
                reader: {
                    type: 'json',
                        ...
                        listeners: {
                            specialkey: function(field, e){
                                ...
                                if (e.getKey() == e.ENTER) {
                                    ...
                                    Ext.Ajax.request({
                                        url: 'api/store/potongan/seragamSave.php',
                                        method: 'POST',
                                        params:{
                                            ...
                                            tempo: me.columns[6].getEditor(me.recordSelected).getSubmitValue()
                                        },
    ...
    hapusRecord: function() {
        ...
        if (selection) {           
            Ext.MessageBox.confirm('Konfirmasi', 'Yakin untuk hapus data yang dipilih?', function(btn,text) {
                if(btn=='yes') {
                    Ext.Ajax.request({
                        url: 'api/store/potongan/seragamDelete.php',
                        method: 'POST',
                        params:{
                            id: selection.get('id')                    
                        },
                        ...
                        failure:function(reseragam, opt){
                            Ext.MessageBox.show({
                                title: 'Gagal',
                                msg:  'Silahkan coba lagi!',
                                buttons: Ext.MessageBox.OK,
