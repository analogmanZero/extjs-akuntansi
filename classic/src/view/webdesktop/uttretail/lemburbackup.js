Ext.define('Admin.view.webdesktop.uttretail.lemburbackup', {

    extend: 'Ext.grid.Panel',
    alias : 'widget.uttretaillemburbackup',
    ...
    constructor: function(c) {
        ...
        me.store = Ext.create('Admin.store.stores', {
            fields: ['id', 'nik', 'nama', 'nik_backup', 'nama_backup', 'keterangan', {name: 'nilai', type: 'float'}, {name: 'tk', type: 'float'}, {name: 'izin', type: 'float'}, {name: 'sakit', type: 'float'}, {name: 'cuti', type: 'float'}, {name: 'backup', type: 'float'}, {name: 'qty', type: 'float'}, {name: 'jumlah', type: 'float'}],
            url: 'api/store/uttretail/lemburbackupStore.php',
            proxy: {
                type: 'memory',
                reader: {
                    type: 'json',
                        ...
                        listeners: {
                            specialkey: function(field, e){
                                ...
                                if (e.getKey() == e.ENTER) {
                                    ...
                                    //me.columns[8].getEditor(me.recordSelected).focus(true, 10);
                                    Ext.Ajax.request({
                                        url: 'api/store/uttretail/lemburbackupSave.php',
                                        method: 'POST',
                                        params:{
                                            ...
                                            nama_backup: me.columns[4].getEditor(me.recordSelected).getSubmitValue(),
    ...
    hapusRecord: function() {
        ...
        if (selection) {           
            Ext.MessageBox.confirm('Konfirmasi', 'Yakin untuk hapus data yang dipilih?', function(btn,text) {
                if(btn=='yes') {
                    Ext.Ajax.request({
                        url: 'api/store/uttretail/lemburbackupDelete.php',
                        method: 'POST',
                        params:{
                            id: selection.get('id')                    
                        },
    ...
    getJumlah: function() {
        ...
        setTimeout(function() {
            ...
            me.columns[8].getEditor(me.recordSelected).setValue(
                Math.floor(eval(nilai)*eval(qty)) 
            );
