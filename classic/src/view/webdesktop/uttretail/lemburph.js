Ext.define('Admin.view.webdesktop.uttretail.lemburph', {

    extend: 'Ext.grid.Panel',
    alias : 'widget.uttretaillemburph',
    ...
    constructor: function(c) {
        ...
        var storeKaryawan = Ext.create('Admin.store.stores', {
            ...
            url: 'api/store/uttretail/karyawanStore.php',
            pageSize: 1000000
        });
        ...
        me.store = Ext.create('Admin.store.stores', {
            fields: ['id', 'nik', 'nama', 'jabatan', 'tanggal',  {name: 'nilai', type: 'float'}, 'masuk', {name: 'qty', type: 'float'}, {name: 'jumlah', type: 'float'}],
            url: 'api/store/uttretail/lemburphStore.php',
            proxy: {
                type: 'memory',
                reader: {
                    type: 'json',
                        ...
                        listeners: {
                            specialkey: function(field, e){
                                ...
                                if (e.getKey() == e.ENTER) {
                                    ...
                                    //me.columns[8].getEditor(me.recordSelected).focus(true, 10);
                                    Ext.Ajax.request({
                                        url: 'api/store/uttretail/lemburphSave.php',
                                        method: 'POST',
                                        params:{
    ...
    hapusRecord: function() {
        ...
        if (selection) {           
            Ext.MessageBox.confirm('Konfirmasi', 'Yakin untuk hapus data yang dipilih?', function(btn,text) {
                if(btn=='yes') {
                    Ext.Ajax.request({
                        url: 'api/store/uttretail/lemburphDelete.php',
                        method: 'POST',
                        params:{
                            id: selection.get('id')                    
                        },
